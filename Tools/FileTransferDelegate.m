...
static void linphone_iphone_file_transfer_recv(LinphoneChatMessage *message, const LinphoneContent *content,
	...
	if (size == 0) {
		DLog(@"Transfer of %s (%d bytes): download finished", linphone_content_get_name(content), size);
		...
		[LinphoneManager.instance.photoLibrary
			writeImageToSavedPhotosAlbum:image.CGImage
						 ...
						 completionBlock:^(NSURL *assetURL, NSError *error) {
						   if (error) {
							   DLog(@"Cannot save image data downloaded [%@]", [error localizedDescription]);
...
//                               [PhoneMainView.instance presentViewController:errView animated:YES completion:nil];
							   ...
							   DLog(@"Image saved to [%@]", [assetURL absoluteString]);
		...
		DLog(@"Transfer of %s (%d bytes): already %ld sent, adding %ld", linphone_content_get_name(content),
...
static LinphoneBuffer *linphone_iphone_file_transfer_send(LinphoneChatMessage *message, const LinphoneContent *content,
	...
	if (thiz.data) {
		...
		DLog(@"Transfer of %s (%d bytes): already sent %ld (%f%%), remaining %ld", linphone_content_get_name(content),
		...
		DLog(@"Transfer of %s (%d bytes): %d Error - no upload data in progress!", linphone_content_get_name(content),
			 total, offset);
	}
...
- (void)upload:(UIImage *)image withURL:(NSURL *)url forChatRoom:(LinphoneChatRoom *)chatRoom withQuality:(float)quality {
	...
	DLog(@"%p Uploading content from message %p", self, _message);
...
- (BOOL)download:(LinphoneChatMessage *)message {
	...
	DLog(@"%p Downloading content in %p from %s", self, message, url);
...
- (void)stopAndDestroy {
	...
	if (_message != NULL) {
		...
		_message = NULL;
		DLog(@"%p Cancelling transfer from %p", self, msg);
