...
- (instancetype)init {
	...
	if (!self) {
		DLog(@"ProviderDelegate not initialized...");
	}
	return self;
}
...
- (void)configAudioSession:(AVAudioSession *)audioSession {
	...
	[audioSession setCategory:AVAudioSessionCategoryPlayAndRecord
				  withOptions:AVAudioSessionCategoryOptionAllowBluetooth
						error:&err];
	if (err) {
		DLog(@"Unable to change audio category because : %@", err.localizedDescription);
		err = nil;
	}
	[audioSession setMode:AVAudioSessionModeVoiceChat error:&err];
	if (err) {
		DLog(@"Unable to change audio mode because : %@", err.localizedDescription);
		err = nil;
	}
	double sampleRate = 44100.0;
	[audioSession setPreferredSampleRate:sampleRate error:&err];
	if (err) {
		DLog(@"Unable to change preferred sample rate because : %@", err.localizedDescription);
		err = nil;
	}
}
...
- (void)reportIncomingCall:(LinphoneCall *) call withUUID:(NSUUID *)uuid handle:(NSString *)handle video:(BOOL)video; {
	...
	DLog(@"CallKit: report new incoming call");
	
	[self.provider reportNewIncomingCallWithUUID:uuid
										  update:update
									  completion:^(NSError *error) {
										  if (error) {
											  DLog(@"CallKit: cannot complete incoming call from [%@] caused by [%@]",handle,[error localizedDescription]);
...
- (void)provider:(CXProvider *)provider performSetHeldCallAction:(nonnull CXSetHeldCallAction *)action {
	DLog(@"CallKit : Call paused status changed");
	[action fulfill];
...
- (void)provider:(CXProvider *)provider didActivateAudioSession:(AVAudioSession *)audioSession {
	DLog(@"CallKit : Audio session activated");
...
- (void)provider:(CXProvider *)provider didDeactivateAudioSession:(nonnull AVAudioSession *)audioSession {
	DLog(@"CallKit : Audio session deactivated");
